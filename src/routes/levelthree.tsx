...
export const Route = createFileRoute("/levelthree")({
  head: () => ({
    meta: [
      { title: "The Storm Shelter — The Resilience Trail" },
      { name: "description", content: "Waypoint 3: Stress Management. Five questions. Drop a stone." },
    ],
  }),
...
const ACCENT = "#66c3b7"; // teal
const TINT   = "#d4efec"; // pastel teal

const STAGE =
  "Wind kicks up. A crooked lean-to offers cover that's better than nothing. The Trailkeeper doesn't rush under it.";

const INTRO_LINES = [
  "Bad news up front — I can't make the storm stop. Nobody can, and anyone who tells you otherwise is selling something.",
  "What I actually want to know is what you do while you're standing in it.",
];

const QUESTIONS = [
  "When something knocks you sideways, how well do you actually recover — not just power through?",
  "Do you build in real recovery time on purpose, or only when you're forced to by collapse?",
  "Do you spend a lot of time flat-out overwhelmed?",
  "Can you disconnect from work and obligations when you need to, or does it follow you into the shower?",
  "Do you have coping strategies that actually hold up under pressure — not just ones you've read about in a listicle?",
];

const OUTRO_ACTION = "(finally ducks under the shelter, waits for you)";
const OUTRO_SPEECH =
  "The stress doesn't go anywhere. It just stops being the thing that flattens you every damn time. That's the whole game here — recovery, not elimination.";
...
function LevelThreePage() {
  ...
  return (
    <div style={{
      minHeight: "100dvh",
      background: `linear-gradient(160deg, ${TINT} 0%, ${LC.offWhite} 50%, ${TINT}88 100%)`,
      display: "grid",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem", paddingLeft: "52px" }}>
            {INTRO_LINES.map((line, i) => (
              <div key={i} style={{
                ...
                fontFamily: "'Fraunces', Georgia, serif", fontStyle: "italic",
                fontSize: "clamp(1.1rem, 2.2vw, 1.35rem)", color: LC.ink, lineHeight: 1.65,
            ...
            {phase === "outro" && (
              <div style={{ marginTop: "1rem" }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end" }}>
                  <button type="button" onClick={() => { sessionStorage.setItem("trail-score-3", String(avg)); navigate({ to: "/levelfour" }); }} style={ctaStyle(ACCENT)}>
                    Walk on →
                  </button>
                </div>
              </div>
      ...
      <div style={{
        position: "relative",
        overflow: "hidden",
        background: `linear-gradient(180deg, ${TINT} 0%, ${TINT}88 100%)`,
        ...
        <img
          ...
          style={{
            ...
            filter: `drop-shadow(0 8px 32px rgba(102,195,183,0.18)) drop-shadow(0 2px 8px rgba(14,80,70,0.10))`,
            opacity: visible ? 1 : 0,
