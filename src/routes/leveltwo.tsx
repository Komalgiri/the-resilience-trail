...
export const Route = createFileRoute("/leveltwo")({
  head: () => ({
    meta: [
      { title: "The Long Night — The Resilience Trail" },
      { name: "description", content: "Waypoint 2: Sleep. Five questions. Drop a stone." },
    ],
  }),
...
const ACCENT = "#ff8f00"; // burnt orange
const TINT   = "#ffdfb4"; // pastel orange

const STAGE =
  "Dusk falls fast, almost rudely fast. A lantern glows next to a bedroll nobody's used in a while.";

const INTRO_LINES = [
  "Sleep. The first thing people torch when life gets loud, and the one they miss the most once it's gone.",
  "I've watched travelers trade it for deadlines, doomscrolling, \"just one more episode\" — every single one of them swore they'd pay it back later. Nobody ever does. Sleep debt doesn't work like a credit card.",
];

const QUESTIONS = [
  "Do you actually wake up feeling rested most mornings, or does the alarm feel like an ambush?",
  "Anything close to a consistent sleep and wake time, or is your schedule feral?",
  "Do you actually treat sleep like it matters, or does it lose to everything else on the list?",
  "Are you getting enough sleep for your body — not the internet's opinion of what's enough?",
  "Is your bedroom built for rest, or is it a second office with a mattress in it?",
];

const OUTRO_ACTION = "(turns the lantern down, not off)";
const OUTRO_SPEECH =
  "Sleep's usually the first thing sacrificed, and the hardest thing to buy back once you have. Chew on that walking forward.";
...
function LevelTwoPage() {
  ...
  return (
    <div style={{
      minHeight: "100dvh",
      background: `linear-gradient(160deg, ${TINT} 0%, ${LC.offWhite} 50%, #fff8ef 100%)`,
      display: "grid",
            ...
            {phase === "outro" && (
              <div style={{ marginTop: "1rem" }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end" }}>
                  <button type="button" onClick={() => { sessionStorage.setItem("trail-score-2", String(avg)); navigate({ to: "/levelthree" }); }} style={ctaStyle(ACCENT)}>
                    Walk on →
                  </button>
                </div>
              </div>
      ...
      <div style={{
        position: "relative",
        overflow: "hidden",
        background: `linear-gradient(180deg, ${TINT} 0%, #fff0d4 100%)`,
        ...
        <img
          ...
          style={{
            ...
            filter: "drop-shadow(0 8px 32px rgba(255,143,0,0.12)) drop-shadow(0 2px 8px rgba(14,80,70,0.08))",
            opacity: visible ? 1 : 0,
